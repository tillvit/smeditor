...
export class UserOptionsWindow extends Window {
  ...
  private createDiv(prefix?: string): HTMLDivElement[] {
    prefix = prefix ?? ""
    let obj = this.getObjects(prefix)
    prefix = prefix == "" ? "" : prefix + "."
    if (!obj) return []
    return Object.entries(obj).filter(entry => !VIEW_BLACKLIST.includes(prefix + entry[0])).map(entry => this.makeObj(prefix!, entry))
  }

  private makeObj(prefix: string, entry: [string, string | number | boolean | object]): HTMLDivElement {
    ...
    if (typeof entry[1] == "object") {
      let dd_icon = document.createElement("img")
      dd_icon.classList.add("icon")
      dd_icon.classList.add("folder-icon")
      dd_icon.src = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABgAAAAYCAYAAADgdz34AAAABmJLR0QA/wD/AP+gvaeTAAAAbklEQVRIie2PMQqAMAxFXwfP6qJVPIjYzcOKdUkhQ0BaCoLkLS3k818CjuN8ztApYzIDl7xvmalFsAIZuIFozKPMMrC0CAJwKIkuGVX5KdkmLEm3ci1JSlLKU49yLSmXVG1es0EANvnvInKcP/AA784fpjlWwNQAAAAASUVORK5CYII="
    ...
    if (typeof entry[1] != "object") {
      let defaultOption = Options.getDefaultOption(prefix + entry[0])!
  ...
  private getObjects(id: string): {[key: string]: string | number | boolean | object} | undefined {
    let path = id.split(".")
    let obj: any = Options
    if (id == "") return obj
