import{v as D,w as H,b as m,x as E,O as f,y as Q,z as I,H as c,J as C,L as e,P as J,a0 as F,Q as s,X as L,Y as v}from"./ChartManager-Ba9C56Ca.js";import{A as d}from"./AnimatedSprite-ic1saOJv.js";import{H as P}from"./HoldBody-Be0k50pL.js";const h="/smeditor/assets/downBodyActive-Bv_L2on1.png",p="/smeditor/assets/downBodyInactive-DJwwHe3Z.png",Y="data:image/png;base64,",k="data:image/png;base64,",z="/smeditor/assets/leftBodyActive-D8U_qbVk.png",b="/smeditor/assets/leftBodyInactive-CuXM3ywY.png",x="data:image/png;base64,",O="data:image/png;base64,",y="/smeditor/assets/rightBodyActive-BoMxf9NU.png",R="/smeditor/assets/rightBodyInactive-DefwkqGq.png",X="data:image/png;base64,",S="data:image/png;base64,",T="data:image/png;base64,",j="/smeditor/assets/upBodyInactive-HRbPnZOL.png",N="data:image/png;base64,",q="data:image/png;base64,",V="data:image/png;base64,",W="data:image/png;base64,",Z="data:image/png;base64,",_="data:image/png;base64,",$="data:image/png;base64,",A0="data:image/png;base64,",n0="data:image/png;base64,",g0="data:image/png;base64,",B0="data:image/png;base64,",t0="data:image/png;base64,",w0="data:image/png;base64,",e0="data:image/png;base64,",o0="data:image/png;base64,",C0="data:image/png;base64,",i0="data:image/png;base64,",a0="data:image/png;base64,",s0="/smeditor/assets/parts-D8TnEx99.png",Q0="data:image/png;base64,",I0="/smeditor/assets/frame-YdtqxpuN.png",c0="/smeditor/assets/parts-CcNv_Je6.png",r0="data:image/png;base64,",G=`precision mediump float;

varying vec2 vUvs;

uniform sampler2D sampler0;
uniform float time;
uniform float quant;

void main() {
  vec4 col = texture2D(
    sampler0,
    vec2(vUvs.x + 0.0625 * quant, mod(vUvs.y - time, 2.0) / 2.0)
  );
  gl_FragColor = col;
}
`,D0=`precision mediump float;

varying vec2 vUvs;

uniform sampler2D sampler0;
uniform float time;
uniform float quant;

void main() {
  vec4 col = texture2D(
    sampler0,
    vec2(mod(vUvs.x - time, 2.0) / 2.0, vUvs.y)
  );
  gl_FragColor = col;
}
`,H0=`precision mediump float;

varying vec2 vUvs;

uniform sampler2D sampler0;
uniform sampler2D sampler1;
uniform float time;

void main() {
  vec4 col = texture2D(sampler0, vUvs);
  if (col.a < 1.0) {
    discard;
  }
  gl_FragColor = texture2D(sampler1, vec2(mod(col.r - time, 1.0), 0.625));
}
`,E0=`precision mediump float;

varying vec2 vUvs;

uniform sampler2D sampler0;

void main() {
  vec4 col = texture2D(sampler0, vUvs);

  gl_FragColor = col;

}
`,r=`precision mediump float;

attribute vec2 aVertexPosition;
attribute vec2 aUvs;

uniform mat3 translationMatrix;
uniform mat3 projectionMatrix;
uniform float time;

varying vec2 vUvs;

float rand(vec2 co) {
  return fract(sin(dot(co, vec2(12.9898, 78.233))) * 43758.5453);
}

void main() {
  vUvs = aUvs;
  gl_Position = vec4(
    (projectionMatrix * translationMatrix * vec3(aVertexPosition.xy, 1.0)).xy,
    0.0,
    1.0
  );

}
`,f0=`78
1.500000 21.000000 0.544734 0.059122
1.500000 28.000000 0.544734 0.000000
-1.500000 28.000000 0.543998 0.000000
-1.500000 21.000000 0.543998 0.059122
-8.000000 21.500000 0.542404 0.054899
-4.000000 18.500000 0.543385 0.080236
-1.500000 4.000000 0.543998 0.202703
0.000000 5.000000 0.544366 0.194257
-4.000000 9.000000 0.543385 0.160473
-4.000000 6.500000 0.543385 0.181588
1.500000 4.000000 0.544734 0.202703
4.000000 6.500000 0.545347 0.181588
4.000000 9.000000 0.545347 0.160473
4.000000 18.500000 0.545347 0.080236
8.000000 21.500000 0.546328 0.054899
4.000000 -5.000000 0.545347 0.278716
19.500000 5.000000 0.549148 0.194257
18.500000 10.000000 0.548903 0.152027
10.000000 0.000000 0.546819 0.236486
8.500000 -6.000000 0.546451 0.287162
5.500000 -6.000000 0.545715 0.287162
0.000000 -9.000000 0.544366 0.312500
1.500000 -10.000000 0.544734 0.320946
-1.500000 -10.000000 0.543998 0.320946
-4.000000 -5.000000 0.543385 0.278716
-5.500000 -6.000000 0.543017 0.287162
-10.000000 0.000000 0.541914 0.236486
-8.500000 -6.000000 0.542282 0.287162
-18.500000 10.000000 0.539829 0.152027
-19.500000 5.000000 0.539584 0.194257
-23.500000 10.000000 0.538603 0.152027
-22.000000 5.000000 0.538971 0.194257
-28.000000 5.500000 0.537500 0.190034
-23.000000 4.000000 0.538726 0.202703
-23.000000 1.500000 0.538726 0.223818
-28.000000 0.500000 0.537500 0.232264
0.000000 -27.500000 0.544366 0.468750
0.000000 -21.500000 0.544366 0.418074
28.000000 0.500000 0.551233 0.232264
23.000000 1.500000 0.550007 0.223818
28.000000 5.500000 0.551233 0.190034
23.000000 4.000000 0.550007 0.202703
23.500000 10.000000 0.550129 0.152027
22.000000 5.000000 0.549761 0.194257
1.500000 21.000000 0.503271 0.001033
-1.500000 21.000000 0.503014 0.001033
-4.000000 18.500000 0.502800 0.001033
4.000000 18.500000 0.503485 0.001033
-4.000000 9.000000 0.502800 0.001033
4.000000 9.000000 0.503485 0.001033
0.000000 5.000000 0.503271 0.001033
1.500000 4.000000 0.510899 0.001033
4.000000 -5.000000 0.511113 0.001033
4.000000 6.500000 0.511113 0.001033
0.000000 -9.000000 0.510899 0.001033
-1.500000 4.000000 0.510642 0.001033
0.000000 -9.000000 0.510642 0.001033
-4.000000 6.500000 0.510428 0.001033
-4.000000 -5.000000 0.510428 0.001033
-1.500000 -10.000000 0.519033 0.001033
0.000000 -21.500000 0.519161 0.001033
1.500000 -10.000000 0.519289 0.001033
8.500000 -6.000000 0.519889 0.001033
-8.500000 -6.000000 0.518433 0.001033
23.000000 4.000000 0.521130 0.001033
22.000000 5.000000 0.521045 0.001033
19.500000 5.000000 0.520831 0.001033
23.000000 1.500000 0.521130 0.001033
-19.500000 5.000000 0.517491 0.001033
-22.000000 5.000000 0.517277 0.001033
-23.000000 4.000000 0.517191 0.001033
-23.000000 1.500000 0.517191 0.001033
5.500000 -6.000000 0.519632 0.001033
-5.500000 -6.000000 0.518690 0.001033
8.000000 0.000000 0.546328 0.236486
-8.000000 0.000000 0.542404 0.236486
-10.000000 1.500000 0.541914 0.223818
10.000000 1.500000 0.546819 0.223818
72
0 1 2
2 3 0
3 2 4
3 4 5
6 7 8
8 9 6
10 7 7
6 10 7
11 12 7
10 11 7
13 14 1
13 1 0
14 13 15
16 17 18
19 16 18
20 19 18
15 20 18
21 22 20
15 21 20
23 22 21
24 25 23
24 23 21
26 25 24
26 27 25
28 29 27
27 26 28
28 30 29
30 31 29
32 33 31
31 30 32
32 34 33
32 35 34
36 37 34
34 35 36
36 38 39
36 39 37
38 40 39
40 41 39
42 43 41
42 41 40
16 43 42
16 42 17
44 45 46
46 47 44
48 49 47
48 47 46
48 50 49
51 52 53
54 52 51
54 51 55
56 55 57
57 58 56
59 60 61
60 62 61
60 59 63
64 65 66
66 67 64
68 69 70
68 70 71
60 63 68
68 71 60
66 62 60
60 67 66
61 62 72
59 73 63
15 18 74
14 15 74
26 24 75
75 24 5
5 4 75
26 76 28
17 77 18`,v0=`34
1.500000 28.000000 0.128125 0.266667
2.000000 30.000000 0.129167 0.250000
-2.000000 30.000000 0.120833 0.250000
-1.500000 28.000000 0.121875 0.266667
8.000000 21.500000 0.141667 0.320833
10.000000 22.000000 0.145833 0.316667
10.000000 0.000000 0.145833 0.500000
18.500000 10.000000 0.163542 0.416667
18.000000 12.000000 0.162500 0.400000
10.000000 4.000000 0.145833 0.466666
23.500000 10.000000 0.173958 0.416667
24.000000 12.000000 0.175000 0.400000
-18.500000 10.000000 0.086458 0.416667
-10.000000 4.000000 0.104167 0.466666
-18.000000 12.000000 0.087500 0.400000
-10.000000 0.000000 0.104167 0.500000
-8.000000 21.500000 0.108333 0.320833
-10.000000 22.000000 0.104167 0.316667
28.000000 5.500000 0.183333 0.454166
30.000000 6.000000 0.187500 0.450000
30.000000 0.000000 0.187500 0.500000
28.000000 0.500000 0.183333 0.495833
0.000000 -27.500000 0.125000 0.729166
0.000000 -30.000000 0.125000 0.750000
-28.000000 0.500000 0.066667 0.495833
-30.000000 0.000000 0.062500 0.500000
-28.000000 5.500000 0.066667 0.454166
-30.000000 6.000000 0.062500 0.450000
-23.500000 10.000000 0.076042 0.416667
-24.000000 12.000000 0.075000 0.400000
8.000000 0.000000 0.141667 0.500000
-8.000000 0.000000 0.108333 0.500000
-10.000000 1.500000 0.104167 0.487500
10.000000 1.500000 0.145833 0.487500
30
0 1 2
3 0 2
4 5 1
1 0 4
5 4 6
7 8 9
10 11 8
8 7 10
12 13 14
15 16 17
16 3 2
16 2 17
18 19 11
11 10 18
20 19 18
18 21 20
22 23 20
20 21 22
22 24 25
25 23 22
26 27 25
25 24 26
28 29 27
27 26 28
12 14 29
29 28 12
4 30 6
15 31 16
32 13 12
7 9 33`,P0=`29
1.500000 21.000000 0.503271 0.001033
-1.500000 21.000000 0.503271 0.001033
-4.000000 18.500000 0.503271 0.001033
4.000000 18.500000 0.503271 0.001033
-4.000000 9.000000 0.503271 0.001033
4.000000 9.000000 0.503271 0.001033
0.000000 5.000000 0.503271 0.001033
1.500000 4.000000 0.510899 0.001033
4.000000 -5.000000 0.510899 0.001033
4.000000 6.500000 0.510899 0.001033
0.000000 -9.000000 0.510899 0.001033
-1.500000 4.000000 0.510899 0.001033
-4.000000 6.500000 0.510899 0.001033
-4.000000 -5.000000 0.510899 0.001033
-1.500000 -10.000000 0.519033 0.001033
0.000000 -21.500000 0.519033 0.001033
1.500000 -10.000000 0.519033 0.001033
8.500000 -6.000000 0.519033 0.001033
-8.500000 -6.000000 0.519033 0.001033
23.000000 4.000000 0.521130 0.001033
22.000000 5.000000 0.521130 0.001033
19.500000 5.000000 0.521130 0.001033
23.000000 1.500000 0.521130 0.001033
-19.500000 5.000000 0.517491 0.001033
-22.000000 5.000000 0.517491 0.001033
-23.000000 4.000000 0.517491 0.001033
-23.000000 1.500000 0.517491 0.001033
5.500000 -6.000000 0.519033 0.001033
-5.500000 -6.000000 0.519033 0.001033
23
0 1 2
2 3 0
4 5 3
4 3 2
4 6 5
7 8 9
10 8 7
10 7 11
10 11 12
12 13 10
14 15 16
15 17 16
15 14 18
19 20 21
21 22 19
23 24 25
23 25 26
15 18 23
23 26 15
21 17 15
15 22 21
16 17 27
14 28 18`,m0=`36
1.500000 28.000000 0.065523 0.068416
2.000000 30.000000 0.063171 0.069380
-2.000000 30.000000 0.087725 0.071097
-1.500000 28.000000 0.085475 0.069820
8.000000 21.500000 0.032200 0.128497
10.000000 22.000000 0.028876 0.147933
10.000000 0.000000 0.011447 0.497174
18.500000 10.000000 0.015941 0.343234
18.000000 12.000000 0.016983 0.314690
10.000000 4.000000 0.014775 0.378867
23.500000 10.000000 0.014987 0.372102
24.000000 12.000000 0.015605 0.353105
-18.500000 10.000000 0.131996 0.348850
-10.000000 4.000000 0.133143 0.384499
-18.000000 12.000000 0.130975 0.320288
-10.000000 0.000000 0.136447 0.502826
-8.000000 21.500000 0.116488 0.133426
-10.000000 22.000000 0.119596 0.153069
28.000000 5.500000 0.013081 0.436815
30.000000 6.000000 0.013111 0.435746
30.000000 0.000000 0.011447 0.497174
28.000000 0.500000 0.011596 0.491616
0.000000 -27.500000 0.072472 0.932979
0.000000 -30.000000 0.072472 0.932979
30.000000 0.000000 0.011625 0.497174
28.000000 0.500000 0.011477 0.491616
-28.000000 0.500000 0.136299 0.497268
-30.000000 0.000000 0.136447 0.502826
-28.000000 5.500000 0.134819 0.442462
-30.000000 6.000000 0.134789 0.441393
-23.500000 10.000000 0.132934 0.377733
-24.000000 12.000000 0.132325 0.358726
8.000000 0.000000 0.011447 0.497174
-8.000000 0.000000 0.136447 0.502826
-10.000000 1.500000 0.135203 0.456484
10.000000 1.500000 0.012695 0.450835
30
0 1 2
3 0 2
4 5 1
1 0 4
5 4 6
7 8 9
10 11 8
8 7 10
12 13 14
15 16 17
16 3 2
16 2 17
18 19 11
11 10 18
20 19 18
18 21 20
22 23 24
24 25 22
22 26 27
27 23 22
28 29 27
27 26 28
30 31 29
29 28 30
12 14 31
31 30 12
4 32 6
15 33 16
34 13 12
7 9 35`,M0=`45
-16.0    0.0  0 0.75
-11.31 -11.31 0 0.75
  0.0  -16.0  0 0.75
 11.31 -11.31 0 0.75
 16.0    0.0  0 0.75
 11.31  11.31 0 0.75
 -0.0   16.0  0 0.75
-11.31  11.31 0 0.75
29.422066 0.009519 -0.999500 0.25
25.401829 14.852878 -0.931249 0.1240015
14.843359 25.411350 -0.751997 0.0343755
-0.000001 29.431583 -0.500000 0.00025
-14.843361 25.411348 -0.248003 0.0343755
-25.401833 14.852878 -0.068751 0.1240015
-29.422066 0.009516 -0.000500 0.25
-25.401827 -14.833844 -0.068751 0.3759985
-14.843357 -25.392317 -0.248003 0.4656245
0.000004 -29.412546 -0.500000 0.49975
14.843363 -25.392307 -0.751997 0.4656245
25.401833 -14.833840 -0.931249 0.3759985
19.921448 0.009518 0.838208 0.25
17.189407 10.096646 0.791826 0.164375
10.087127 17.198929 0.671250 0.104087
-0.000000 19.930965 0.500000 0.080896
-10.087129 17.198925 0.328750 0.104087
-17.189411 10.096646 0.208174 0.164375
-19.921448 0.009518 0.161792 0.25
-17.189405 -10.077612 0.208174 0.335625
-10.087127 -17.179895 0.328750 0.395913
0.000001 -19.911928 0.500000 0.419104
10.087130 -17.179886 0.671250 0.395913
17.189411 -10.077609 0.791826 0.335625
10.420829 0.009516 -0.676915 0.25
8.976986 5.340415 -0.652403 0.2047485
5.330894 8.986506 -0.590503 0.1737985
0.000001 10.430347 -0.500000 0.1615425
-5.330897 8.986505 -0.409497 0.1737985
-8.976989 5.340414 -0.347597 0.2047485
-10.420829 0.009520 -0.323085 0.25
-8.976985 -5.321381 -0.347597 0.2952515
-5.330895 -8.967473 -0.409497 0.3262015
-0.000003 -10.411310 -0.500000 0.3384575
5.330898 -8.967464 -0.590503 0.3262015
8.976988 -5.321376 -0.652403 0.2952515
0 0 0.5 0.75
56
44 0 1
44 1 2
44 2 3
44 3 4
44 4 5
44 5 6
44 6 7
44 7 0
8 9 20
9 21 20
9 10 21
10 22 21
10 11 22
11 23 22
11 12 23
12 24 23
12 13 24
13 25 24
13 14 25
14 26 25
14 15 26
15 27 26
15 16 27
16 28 27
16 17 28
17 29 28
17 18 29
18 30 29
18 19 30
19 31 30
19 8 31 
8 20 31 
20 21 32
21 33 32
21 22 33
22 34 33
22 23 34
23 35 34
23 24 35
24 36 35
24 25 36
25 37 36
25 26 37
26 38 37
26 27 38
27 39 38
27 28 39
28 40 39
28 29 40
29 41 40
29 30 41
30 42 41
30 31 42
31 43 42
31 20 43
20 32 43`,l0=e.from(I0);class t{static arrowPartsTex=D.from(r0,{mipmap:H.OFF});static minePartsTex=D.from(c0,{mipmap:H.OFF});static liftPartsTex=D.from(s0,{mipmap:H.OFF});static liftQuantsTex=D.from(Q0,{mipmap:H.OFF});static arrowBodyGeom;static arrowFrameGeom;static liftBodyGeom;static liftFrameGeom;static mineBodyGeom;static arrowFrameTex;static arrowFrame;static arrowTex;static arrowContainer=new m;static liftTex;static liftContainer=new m;static mineTex;static mineContainer=new m;static loaded=!1;static async initArrowTex(){if(!this.loaded){t.arrowFrameTex=E.create({width:64,height:64,resolution:f.performance.resolution}),t.arrowTex=E.create({width:256,height:320,resolution:f.performance.resolution}),t.liftTex=E.create({width:256,height:320,resolution:f.performance.resolution}),t.mineTex=E.create({width:64,height:64,resolution:f.performance.resolution}),this.arrowBodyGeom=await Q(f0),this.arrowFrameGeom=await Q(v0),this.mineBodyGeom=await Q(M0),this.liftBodyGeom=await Q(P0),this.liftFrameGeom=await Q(m0);{const A=I.from(r,E0,{sampler0:this.arrowPartsTex}),B=new c(t.arrowFrameGeom,A);B.x=32,B.y=32,B.rotation=Math.PI,this.arrowFrame=B}for(let A=0;A<10;A++){const B=I.from(r,G,{sampler0:this.arrowPartsTex,time:0,quant:Math.min(A,6)}),g=new C(t.arrowFrameTex);g.x=A%3*64,g.y=Math.floor(A/3)*64;const w=new c(t.arrowBodyGeom,B);w.x=A%3*64+32,w.y=Math.floor(A/3)*64+32,w.rotation=Math.PI,w.name="body"+A,t.arrowContainer.addChild(g),t.arrowContainer.addChild(w)}for(let A=0;A<10;A++){const B=I.from(r,G,{sampler0:this.liftQuantsTex,time:0,quant:Math.min(A,6)}),g=I.from(r,D0,{sampler0:this.liftPartsTex}),w=new c(t.liftBodyGeom,B);w.x=A%3*64+32,w.y=Math.floor(A/3)*64+32,w.rotation=Math.PI,w.name="body"+A,t.liftContainer.addChild(w);const o=new c(t.liftFrameGeom,g);o.x=A%3*64+32,o.y=Math.floor(A/3)*64+32,o.rotation=Math.PI,o.name="frame"+A,t.liftContainer.addChild(o)}{const A=I.from(r,H0,{sampler0:this.minePartsTex,time:0}),B=new c(t.mineBodyGeom,A),g=new C(l0);g.width=64,g.height=64,g.anchor.set(.5),g.pivot.y=3,t.mineContainer.position.set(32),t.mineContainer.addChild(B),t.mineContainer.addChild(g)}this.loaded=!0}}static setArrowTexTime(A){if(!this.loaded)return;const B=A.chartManager.chartView.getVisualBeat(),g=A.chartManager.chartView.getVisualTime();for(let w=0;w<10;w++){const o=t.arrowContainer.getChildByName("body"+w);o.shader.uniforms.time=B/4;const u=t.liftContainer.getChildByName("body"+w);u.shader.uniforms.time=B/4;const U=t.liftContainer.getChildByName("frame"+w);U.shader.uniforms.time=B/4}t.mineContainer.children[0].shader.uniforms.time=g,t.mineContainer.rotation=g%1*Math.PI*2,A.renderer.render(t.arrowFrame,{renderTexture:t.arrowFrameTex}),A.renderer.render(t.arrowContainer,{renderTexture:t.arrowTex}),A.renderer.render(t.mineContainer,{renderTexture:t.mineTex}),A.renderer.render(t.liftContainer,{renderTexture:t.liftTex})}static setNoteTex(A,B){if(B!==void 0&&B.type=="Mine")A.texture=t.mineTex;else if(B!==void 0&&B.type=="Lift"){const g=[4,8,12,16,24,32,48,64,96,192].indexOf(B?.quant??4);A.texture=new e(t.liftTex.baseTexture,new J(g%3*64,Math.floor(g/3)*64,64,64))}else{const g=[4,8,12,16,24,32,48,64,96,192].indexOf(B?.quant??4);A.texture=new e(t.arrowTex.baseTexture,new J(g%3*64,Math.floor(g/3)*64,64,64))}}}const J0="/smeditor/assets/flashBright-WBniwOZj.png",G0="data:image/png;base64,",K0="/smeditor/assets/hold-BtPJfgG_.png",F0="/smeditor/assets/mine-CPFI4p0C.png",d0=e.from(J0),u0=e.from(G0),U0=e.from(K0),L0=e.from(F0),M={0:{tint:16777215,alpha:1,"scale.x":.8,"scale.y":.8},"0.5":{tint:16777215,alpha:1,"scale.x":1,"scale.y":1},1:{tint:16777215,alpha:0,"scale.x":1,"scale.y":1}},K=n=>({0:{tint:n,alpha:1,"scale.x":.4,"scale.y":.4},"0.5":{tint:n,alpha:1,"scale.x":.5,"scale.y":.5},1:{tint:n,alpha:0,"scale.x":.5,"scale.y":.5}}),l={w0:M,w1:M,w2:K(16777036),w3:K(65382),held:M};class h0 extends m{holdExplosion=new d(F(U0,4,1,160,160)[0]);bright=new C(d0);dim=new C(u0);anims=new Set;constructor(A,B){super(),A.on(this,"hit",g=>{if(B==g.columnNumber){let w=this.dim;if(g.judgement.id=="w0"&&(w=this.bright),l[g.judgement.id]===void 0)return;this.anims.forEach(o=>s.finish(o)),this.anims.clear(),this.anims.add(s.animate(w,l[g.judgement.id],.12))}}),A.on(this,"held",g=>{B==g.columnNumber&&(this.anims.forEach(w=>s.finish(w)),this.anims.clear(),this.anims.add(s.animate(this.bright,l.held,.12)))}),A.on(this,"holdon",g=>{B==g.columnNumber&&(this.holdExplosion.visible=!0)}),A.on(this,"holdoff",g=>{B==g.columnNumber&&(this.holdExplosion.visible=!1)}),A.on(this,"rollon",g=>{B==g.columnNumber&&(this.holdExplosion.visible=!0)}),A.on(this,"rolloff",g=>{B==g.columnNumber&&(this.holdExplosion.visible=!1)}),A.on(this,"hitmine",g=>{if(B==g.columnNumber){const w=new C(L0);w.alpha=0,w.anchor.set(.5),w.blendMode=L.ADD,this.addChild(w),s.animate(w,{0:{alpha:1,rotation:0},"0.5":{alpha:1,rotation:90*Math.PI/180},1:{alpha:0,rotation:180*Math.PI/180}},.4,void 0,()=>w.destroy())}}),this.holdExplosion.visible=!1,this.holdExplosion.anchor.set(.5),this.holdExplosion.scale.set(.5),this.holdExplosion.play(),this.addChild(this.holdExplosion),this.dim.scale.set(.5),this.dim.anchor.set(.5),this.dim.alpha=0,this.addChild(this.dim),this.bright.scale.set(.5),this.bright.anchor.set(.5),this.bright.alpha=0,this.addChild(this.bright)}}const p0="/smeditor/assets/receptor-BthBTozT.png",Y0=e.from(p0),i={},a={};for(const n of["Left","Down","Up","Right"])for(const A of["Body","BottomCap"])for(const B of["Active","Inactive"])i[n]===void 0&&(i[n]={}),i[n][B]===void 0&&(i[n][B]={}),i[n][B][A]=e.from(new URL(Object.assign({"./hold/downBodyActive.png":h,"./hold/downBodyInactive.png":p,"./hold/downBottomCapActive.png":Y,"./hold/downBottomCapInactive.png":k,"./hold/leftBodyActive.png":z,"./hold/leftBodyInactive.png":b,"./hold/leftBottomCapActive.png":x,"./hold/leftBottomCapInactive.png":O,"./hold/rightBodyActive.png":y,"./hold/rightBodyInactive.png":R,"./hold/rightBottomCapActive.png":X,"./hold/rightBottomCapInactive.png":S,"./hold/upBodyActive.png":T,"./hold/upBodyInactive.png":j,"./hold/upBottomCapActive.png":N,"./hold/upBottomCapInactive.png":q})[`./hold/${n.toLowerCase()}${A}${B}.png`],import.meta.url).href),a[n]===void 0&&(a[n]={}),a[n][B]===void 0&&(a[n][B]={}),a[n][B][A]=e.from(new URL(Object.assign({"./roll/downBodyActive.png":V,"./roll/downBodyInactive.png":W,"./roll/downBottomCapActive.png":Z,"./roll/downBottomCapInactive.png":_,"./roll/leftBodyActive.png":$,"./roll/leftBodyInactive.png":A0,"./roll/leftBottomCapActive.png":n0,"./roll/leftBottomCapInactive.png":g0,"./roll/rightBodyActive.png":B0,"./roll/rightBodyInactive.png":t0,"./roll/rightBottomCapActive.png":w0,"./roll/rightBottomCapInactive.png":e0,"./roll/upBodyActive.png":o0,"./roll/upBodyInactive.png":C0,"./roll/upBottomCapActive.png":i0,"./roll/upBottomCapInactive.png":a0})[`./roll/${n.toLowerCase()}${A}${B}.png`],import.meta.url).href);const k0={Left:90,Down:0,Up:180,Right:-90,UpLeft:135,UpRight:-135,DownRight:-45,DownLeft:45},z0=["Receptor","Tap","Lift","Fake","Hold Inactive Head","Hold Active Head","Roll Inactive Head","Roll Active Head","NoteFlash"],y0={elements:{Left:{Receptor:n=>{let A;const B=F(Y0,2,1,256,256),g=new d(B[0]);return g.width=64,g.height=64,g.anchor.set(.5),n.noteskin.on(g,"ghosttap",w=>{w.columnNumber==n.columnNumber&&(s.finish(A),A=s.animate(g,{0:{alpha:1.2,width:48,height:48},1:{alpha:1,width:64,height:64}},.06))}),n.noteskin.onUpdate(g,w=>{const o=(w.getVisualBeat()%1+1)%1;g.currentFrame=o<.2?0:1}),g},Tap:n=>{const A=new C(e.WHITE);return t.setNoteTex(A,n.note),A.anchor.set(.5),A},NoteFlash:n=>new h0(n.noteskin,n.columnNumber),Fake:{element:"Tap"},Lift:{element:"Tap"},Mine:{element:"Tap"},"Hold Active Head":{element:"Tap"},"Hold Inactive Head":{element:"Tap"},"Hold Active Body":n=>new P(i[n.columnName].Active.Body),"Hold Inactive Body":n=>new P(i[n.columnName].Inactive.Body),"Hold Active TopCap":()=>new C(e.EMPTY),"Hold Inactive TopCap":()=>new C(e.EMPTY),"Hold Active BottomCap":n=>new v(i[n.columnName].Active.BottomCap),"Hold Inactive BottomCap":n=>new v(i[n.columnName].Inactive.BottomCap),"Roll Active Head":{element:"Tap"},"Roll Inactive Head":{element:"Tap"},"Roll Active Body":n=>new P(a[n.columnName].Active.Body),"Roll Inactive Body":n=>new P(a[n.columnName].Inactive.Body),"Roll Active TopCap":()=>new C(e.EMPTY),"Roll Inactive TopCap":()=>new C(e.EMPTY),"Roll Active BottomCap":n=>new v(a[n.columnName].Active.BottomCap),"Roll Inactive BottomCap":n=>new v(a[n.columnName].Inactive.BottomCap)}},load:function(n,A){const B=n.columnName;n.columnName="Left";const g=this.loadElement(n,A);return z0.includes(n.element)&&(g.rotation=k0[B]*Math.PI/180),g},init(){t.initArrowTex()},update(n){t.setArrowTexTime(n.chartManager.app)},metrics:{HoldBodyBottomOffset:-32,RollBodyBottomOffset:-32},hideIcons:["Lift"]};export{y0 as default};
