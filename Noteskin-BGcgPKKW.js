import{am as I,an as E,C as P,U as v,O as m,ao as w,ap as c,a as Q,X as a,T as e,aa as h,aq as L,B as x}from"./Simfile-B3_kH-9l.js";import{A as H}from"./AnimatedSprite-CQpssVBH.js";import{B as C,H as l}from"./app.js";import{H as d}from"./HoldBody-CG26JuPz.js";const b="/smeditor/assets/downBodyActive-Bv_L2on1.png",G="/smeditor/assets/downBodyInactive-DJwwHe3Z.png",S="data:image/png;base64,",y="data:image/png;base64,",Y="/smeditor/assets/leftBodyActive-D8U_qbVk.png",N="/smeditor/assets/leftBodyInactive-CuXM3ywY.png",V="data:image/png;base64,",O="data:image/png;base64,",X="/smeditor/assets/rightBodyActive-BoMxf9NU.png",K="/smeditor/assets/rightBodyInactive-DefwkqGq.png",k="data:image/png;base64,",R="data:image/png;base64,",M="data:image/png;base64,",Z="/smeditor/assets/upBodyInactive-HRbPnZOL.png",J="data:image/png;base64,",z="data:image/png;base64,",q="/smeditor/assets/upleftBodyActive-D0LHSYES.png",j="/smeditor/assets/upleftBodyInactive-CEQ5btFq.png",W="data:image/png;base64,",_="data:image/png;base64,",$="/smeditor/assets/uprightBodyActive-h5yylfso.png",A0="/smeditor/assets/uprightBodyInactive-DFwkN_l0.png",n0="data:image/png;base64,",t0="data:image/png;base64,",o0="/smeditor/assets/bodyActive-DqraHWXw.png",g0="/smeditor/assets/bodyInactive-DbHJiYgp.png",r0="data:image/png;base64,",e0="data:image/png;base64,",B0="/smeditor/assets/parts-D8TnEx99.png",a0="data:image/png;base64,",i0="/smeditor/assets/frame-YdtqxpuN.png",C0="/smeditor/assets/parts-CcNv_Je6.png",s0="data:image/png;base64,",u=`precision mediump float;

varying vec2 vUvs;

uniform sampler2D sampler0;
uniform float time;
uniform float quant;

void main() {
  vec4 col = texture2D(
    sampler0,
    vec2(vUvs.x + 0.0625 * quant, mod(vUvs.y - time, 2.0) / 2.0)
  );
  gl_FragColor = col;
}
`,w0=`precision mediump float;

varying vec2 vUvs;

uniform sampler2D sampler0;
uniform float time;
uniform float quant;

void main() {
  vec4 col = texture2D(
    sampler0,
    vec2(mod(vUvs.x - time, 2.0) / 2.0, vUvs.y)
  );
  gl_FragColor = col;
}
`,c0=`precision mediump float;

varying vec2 vUvs;

uniform sampler2D sampler0;
uniform sampler2D sampler1;
uniform float time;

void main() {
  vec4 col = texture2D(sampler0, vUvs);
  if (col.a < 1.0) {
    discard;
  }
  gl_FragColor = texture2D(sampler1, vec2(mod(col.r - time, 1.0), 0.625));
}
`,Q0=`precision mediump float;

varying vec2 vUvs;

uniform sampler2D sampler0;

void main() {
  vec4 col = texture2D(sampler0, vUvs);

  gl_FragColor = col;

}
`,f=`precision mediump float;

attribute vec2 aVertexPosition;
attribute vec2 aUvs;

uniform mat3 translationMatrix;
uniform mat3 projectionMatrix;
uniform float time;

varying vec2 vUvs;

float rand(vec2 co) {
  return fract(sin(dot(co, vec2(12.9898, 78.233))) * 43758.5453);
}

void main() {
  vUvs = aUvs;
  gl_Position = vec4(
    (projectionMatrix * translationMatrix * vec3(aVertexPosition.xy, 1.0)).xy,
    0.0,
    1.0
  );

}
`,f0=`78\r
1.500000 21.000000 0.544734 0.059122\r
1.500000 28.000000 0.544734 0.000000\r
-1.500000 28.000000 0.543998 0.000000\r
-1.500000 21.000000 0.543998 0.059122\r
-8.000000 21.500000 0.542404 0.054899\r
-4.000000 18.500000 0.543385 0.080236\r
-1.500000 4.000000 0.543998 0.202703\r
0.000000 5.000000 0.544366 0.194257\r
-4.000000 9.000000 0.543385 0.160473\r
-4.000000 6.500000 0.543385 0.181588\r
1.500000 4.000000 0.544734 0.202703\r
4.000000 6.500000 0.545347 0.181588\r
4.000000 9.000000 0.545347 0.160473\r
4.000000 18.500000 0.545347 0.080236\r
8.000000 21.500000 0.546328 0.054899\r
4.000000 -5.000000 0.545347 0.278716\r
19.500000 5.000000 0.549148 0.194257\r
18.500000 10.000000 0.548903 0.152027\r
10.000000 0.000000 0.546819 0.236486\r
8.500000 -6.000000 0.546451 0.287162\r
5.500000 -6.000000 0.545715 0.287162\r
0.000000 -9.000000 0.544366 0.312500\r
1.500000 -10.000000 0.544734 0.320946\r
-1.500000 -10.000000 0.543998 0.320946\r
-4.000000 -5.000000 0.543385 0.278716\r
-5.500000 -6.000000 0.543017 0.287162\r
-10.000000 0.000000 0.541914 0.236486\r
-8.500000 -6.000000 0.542282 0.287162\r
-18.500000 10.000000 0.539829 0.152027\r
-19.500000 5.000000 0.539584 0.194257\r
-23.500000 10.000000 0.538603 0.152027\r
-22.000000 5.000000 0.538971 0.194257\r
-28.000000 5.500000 0.537500 0.190034\r
-23.000000 4.000000 0.538726 0.202703\r
-23.000000 1.500000 0.538726 0.223818\r
-28.000000 0.500000 0.537500 0.232264\r
0.000000 -27.500000 0.544366 0.468750\r
0.000000 -21.500000 0.544366 0.418074\r
28.000000 0.500000 0.551233 0.232264\r
23.000000 1.500000 0.550007 0.223818\r
28.000000 5.500000 0.551233 0.190034\r
23.000000 4.000000 0.550007 0.202703\r
23.500000 10.000000 0.550129 0.152027\r
22.000000 5.000000 0.549761 0.194257\r
1.500000 21.000000 0.503271 0.001033\r
-1.500000 21.000000 0.503014 0.001033\r
-4.000000 18.500000 0.502800 0.001033\r
4.000000 18.500000 0.503485 0.001033\r
-4.000000 9.000000 0.502800 0.001033\r
4.000000 9.000000 0.503485 0.001033\r
0.000000 5.000000 0.503271 0.001033\r
1.500000 4.000000 0.510899 0.001033\r
4.000000 -5.000000 0.511113 0.001033\r
4.000000 6.500000 0.511113 0.001033\r
0.000000 -9.000000 0.510899 0.001033\r
-1.500000 4.000000 0.510642 0.001033\r
0.000000 -9.000000 0.510642 0.001033\r
-4.000000 6.500000 0.510428 0.001033\r
-4.000000 -5.000000 0.510428 0.001033\r
-1.500000 -10.000000 0.519033 0.001033\r
0.000000 -21.500000 0.519161 0.001033\r
1.500000 -10.000000 0.519289 0.001033\r
8.500000 -6.000000 0.519889 0.001033\r
-8.500000 -6.000000 0.518433 0.001033\r
23.000000 4.000000 0.521130 0.001033\r
22.000000 5.000000 0.521045 0.001033\r
19.500000 5.000000 0.520831 0.001033\r
23.000000 1.500000 0.521130 0.001033\r
-19.500000 5.000000 0.517491 0.001033\r
-22.000000 5.000000 0.517277 0.001033\r
-23.000000 4.000000 0.517191 0.001033\r
-23.000000 1.500000 0.517191 0.001033\r
5.500000 -6.000000 0.519632 0.001033\r
-5.500000 -6.000000 0.518690 0.001033\r
8.000000 0.000000 0.546328 0.236486\r
-8.000000 0.000000 0.542404 0.236486\r
-10.000000 1.500000 0.541914 0.223818\r
10.000000 1.500000 0.546819 0.223818\r
72\r
0 1 2\r
2 3 0\r
3 2 4\r
3 4 5\r
6 7 8\r
8 9 6\r
10 7 7\r
6 10 7\r
11 12 7\r
10 11 7\r
13 14 1\r
13 1 0\r
14 13 15\r
16 17 18\r
19 16 18\r
20 19 18\r
15 20 18\r
21 22 20\r
15 21 20\r
23 22 21\r
24 25 23\r
24 23 21\r
26 25 24\r
26 27 25\r
28 29 27\r
27 26 28\r
28 30 29\r
30 31 29\r
32 33 31\r
31 30 32\r
32 34 33\r
32 35 34\r
36 37 34\r
34 35 36\r
36 38 39\r
36 39 37\r
38 40 39\r
40 41 39\r
42 43 41\r
42 41 40\r
16 43 42\r
16 42 17\r
44 45 46\r
46 47 44\r
48 49 47\r
48 47 46\r
48 50 49\r
51 52 53\r
54 52 51\r
54 51 55\r
56 55 57\r
57 58 56\r
59 60 61\r
60 62 61\r
60 59 63\r
64 65 66\r
66 67 64\r
68 69 70\r
68 70 71\r
60 63 68\r
68 71 60\r
66 62 60\r
60 67 66\r
61 62 72\r
59 73 63\r
15 18 74\r
14 15 74\r
26 24 75\r
75 24 5\r
5 4 75\r
26 76 28\r
17 77 18\r
`,I0=`34\r
1.500000 28.000000 0.128125 0.266667\r
2.000000 30.000000 0.129167 0.250000\r
-2.000000 30.000000 0.120833 0.250000\r
-1.500000 28.000000 0.121875 0.266667\r
8.000000 21.500000 0.141667 0.320833\r
10.000000 22.000000 0.145833 0.316667\r
10.000000 0.000000 0.145833 0.500000\r
18.500000 10.000000 0.163542 0.416667\r
18.000000 12.000000 0.162500 0.400000\r
10.000000 4.000000 0.145833 0.466666\r
23.500000 10.000000 0.173958 0.416667\r
24.000000 12.000000 0.175000 0.400000\r
-18.500000 10.000000 0.086458 0.416667\r
-10.000000 4.000000 0.104167 0.466666\r
-18.000000 12.000000 0.087500 0.400000\r
-10.000000 0.000000 0.104167 0.500000\r
-8.000000 21.500000 0.108333 0.320833\r
-10.000000 22.000000 0.104167 0.316667\r
28.000000 5.500000 0.183333 0.454166\r
30.000000 6.000000 0.187500 0.450000\r
30.000000 0.000000 0.187500 0.500000\r
28.000000 0.500000 0.183333 0.495833\r
0.000000 -27.500000 0.125000 0.729166\r
0.000000 -30.000000 0.125000 0.750000\r
-28.000000 0.500000 0.066667 0.495833\r
-30.000000 0.000000 0.062500 0.500000\r
-28.000000 5.500000 0.066667 0.454166\r
-30.000000 6.000000 0.062500 0.450000\r
-23.500000 10.000000 0.076042 0.416667\r
-24.000000 12.000000 0.075000 0.400000\r
8.000000 0.000000 0.141667 0.500000\r
-8.000000 0.000000 0.108333 0.500000\r
-10.000000 1.500000 0.104167 0.487500\r
10.000000 1.500000 0.145833 0.487500\r
30\r
0 1 2\r
3 0 2\r
4 5 1\r
1 0 4\r
5 4 6\r
7 8 9\r
10 11 8\r
8 7 10\r
12 13 14\r
15 16 17\r
16 3 2\r
16 2 17\r
18 19 11\r
11 10 18\r
20 19 18\r
18 21 20\r
22 23 20\r
20 21 22\r
22 24 25\r
25 23 22\r
26 27 25\r
25 24 26\r
28 29 27\r
27 26 28\r
12 14 29\r
29 28 12\r
4 30 6\r
15 31 16\r
32 13 12\r
7 9 33`,E0=`29
1.500000 21.000000 0.503271 0.001033
-1.500000 21.000000 0.503271 0.001033
-4.000000 18.500000 0.503271 0.001033
4.000000 18.500000 0.503271 0.001033
-4.000000 9.000000 0.503271 0.001033
4.000000 9.000000 0.503271 0.001033
0.000000 5.000000 0.503271 0.001033
1.500000 4.000000 0.510899 0.001033
4.000000 -5.000000 0.510899 0.001033
4.000000 6.500000 0.510899 0.001033
0.000000 -9.000000 0.510899 0.001033
-1.500000 4.000000 0.510899 0.001033
-4.000000 6.500000 0.510899 0.001033
-4.000000 -5.000000 0.510899 0.001033
-1.500000 -10.000000 0.519033 0.001033
0.000000 -21.500000 0.519033 0.001033
1.500000 -10.000000 0.519033 0.001033
8.500000 -6.000000 0.519033 0.001033
-8.500000 -6.000000 0.519033 0.001033
23.000000 4.000000 0.521130 0.001033
22.000000 5.000000 0.521130 0.001033
19.500000 5.000000 0.521130 0.001033
23.000000 1.500000 0.521130 0.001033
-19.500000 5.000000 0.517491 0.001033
-22.000000 5.000000 0.517491 0.001033
-23.000000 4.000000 0.517491 0.001033
-23.000000 1.500000 0.517491 0.001033
5.500000 -6.000000 0.519033 0.001033
-5.500000 -6.000000 0.519033 0.001033
23
0 1 2
2 3 0
4 5 3
4 3 2
4 6 5
7 8 9
10 8 7
10 7 11
10 11 12
12 13 10
14 15 16
15 17 16
15 14 18
19 20 21
21 22 19
23 24 25
23 25 26
15 18 23
23 26 15
21 17 15
15 22 21
16 17 27
14 28 18
`,v0=`36
1.500000 28.000000 0.065523 0.068416
2.000000 30.000000 0.063171 0.069380
-2.000000 30.000000 0.087725 0.071097
-1.500000 28.000000 0.085475 0.069820
8.000000 21.500000 0.032200 0.128497
10.000000 22.000000 0.028876 0.147933
10.000000 0.000000 0.011447 0.497174
18.500000 10.000000 0.015941 0.343234
18.000000 12.000000 0.016983 0.314690
10.000000 4.000000 0.014775 0.378867
23.500000 10.000000 0.014987 0.372102
24.000000 12.000000 0.015605 0.353105
-18.500000 10.000000 0.131996 0.348850
-10.000000 4.000000 0.133143 0.384499
-18.000000 12.000000 0.130975 0.320288
-10.000000 0.000000 0.136447 0.502826
-8.000000 21.500000 0.116488 0.133426
-10.000000 22.000000 0.119596 0.153069
28.000000 5.500000 0.013081 0.436815
30.000000 6.000000 0.013111 0.435746
30.000000 0.000000 0.011447 0.497174
28.000000 0.500000 0.011596 0.491616
0.000000 -27.500000 0.072472 0.932979
0.000000 -30.000000 0.072472 0.932979
30.000000 0.000000 0.011625 0.497174
28.000000 0.500000 0.011477 0.491616
-28.000000 0.500000 0.136299 0.497268
-30.000000 0.000000 0.136447 0.502826
-28.000000 5.500000 0.134819 0.442462
-30.000000 6.000000 0.134789 0.441393
-23.500000 10.000000 0.132934 0.377733
-24.000000 12.000000 0.132325 0.358726
8.000000 0.000000 0.011447 0.497174
-8.000000 0.000000 0.136447 0.502826
-10.000000 1.500000 0.135203 0.456484
10.000000 1.500000 0.012695 0.450835
30
0 1 2
3 0 2
4 5 1
1 0 4
5 4 6
7 8 9
10 11 8
8 7 10
12 13 14
15 16 17
16 3 2
16 2 17
18 19 11
11 10 18
20 19 18
18 21 20
22 23 24
24 25 22
22 26 27
27 23 22
28 29 27
27 26 28
30 31 29
29 28 30
12 14 31
31 30 12
4 32 6
15 33 16
34 13 12
7 9 35
`,m0=`45
-16.0    0.0  0 0.75
-11.31 -11.31 0 0.75
  0.0  -16.0  0 0.75
 11.31 -11.31 0 0.75
 16.0    0.0  0 0.75
 11.31  11.31 0 0.75
 -0.0   16.0  0 0.75
-11.31  11.31 0 0.75
29.422066 0.009519 -0.999500 0.25
25.401829 14.852878 -0.931249 0.1240015
14.843359 25.411350 -0.751997 0.0343755
-0.000001 29.431583 -0.500000 0.00025
-14.843361 25.411348 -0.248003 0.0343755
-25.401833 14.852878 -0.068751 0.1240015
-29.422066 0.009516 -0.000500 0.25
-25.401827 -14.833844 -0.068751 0.3759985
-14.843357 -25.392317 -0.248003 0.4656245
0.000004 -29.412546 -0.500000 0.49975
14.843363 -25.392307 -0.751997 0.4656245
25.401833 -14.833840 -0.931249 0.3759985
19.921448 0.009518 0.838208 0.25
17.189407 10.096646 0.791826 0.164375
10.087127 17.198929 0.671250 0.104087
-0.000000 19.930965 0.500000 0.080896
-10.087129 17.198925 0.328750 0.104087
-17.189411 10.096646 0.208174 0.164375
-19.921448 0.009518 0.161792 0.25
-17.189405 -10.077612 0.208174 0.335625
-10.087127 -17.179895 0.328750 0.395913
0.000001 -19.911928 0.500000 0.419104
10.087130 -17.179886 0.671250 0.395913
17.189411 -10.077609 0.791826 0.335625
10.420829 0.009516 -0.676915 0.25
8.976986 5.340415 -0.652403 0.2047485
5.330894 8.986506 -0.590503 0.1737985
0.000001 10.430347 -0.500000 0.1615425
-5.330897 8.986505 -0.409497 0.1737985
-8.976989 5.340414 -0.347597 0.2047485
-10.420829 0.009520 -0.323085 0.25
-8.976985 -5.321381 -0.347597 0.2952515
-5.330895 -8.967473 -0.409497 0.3262015
-0.000003 -10.411310 -0.500000 0.3384575
5.330898 -8.967464 -0.590503 0.3262015
8.976988 -5.321376 -0.652403 0.2952515
0 0 0.5 0.75
56
44 0 1
44 1 2
44 2 3
44 3 4
44 4 5
44 5 6
44 6 7
44 7 0
8 9 20
9 21 20
9 10 21
10 22 21
10 11 22
11 23 22
11 12 23
12 24 23
12 13 24
13 25 24
13 14 25
14 26 25
14 15 26
15 27 26
15 16 27
16 28 27
16 17 28
17 29 28
17 18 29
18 30 29
18 19 30
19 31 30
19 8 31 
8 20 31 
20 21 32
21 33 32
21 22 33
22 34 33
22 23 34
23 35 34
23 24 35
24 36 35
24 25 36
25 37 36
25 26 37
26 38 37
26 27 38
27 39 38
27 28 39
28 40 39
28 29 40
29 41 40
29 30 41
30 42 41
30 31 42
31 43 42
31 20 43
20 32 43`,l0=e.from(i0);class g{static arrowPartsTex=I.from(s0,{mipmap:E.OFF});static minePartsTex=I.from(C0,{mipmap:E.OFF});static liftPartsTex=I.from(B0,{mipmap:E.OFF});static liftQuantsTex=I.from(a0,{mipmap:E.OFF});static arrowBodyGeom;static arrowFrameGeom;static liftBodyGeom;static liftFrameGeom;static mineBodyGeom;static arrowFrameTex;static arrowFrame;static arrowTex;static arrowContainer=new P;static liftTex;static liftContainer=new P;static mineTex;static mineContainer=new P;static loaded=!1;static async initArrowTex(){if(!this.loaded){g.arrowFrameTex=v.create({width:64,height:64,resolution:m.performance.resolution}),g.arrowTex=v.create({width:256,height:320,resolution:m.performance.resolution}),g.liftTex=v.create({width:256,height:320,resolution:m.performance.resolution}),g.mineTex=v.create({width:64,height:64,resolution:m.performance.resolution}),this.arrowBodyGeom=await w(f0),this.arrowFrameGeom=await w(I0),this.mineBodyGeom=await w(m0),this.liftBodyGeom=await w(E0),this.liftFrameGeom=await w(v0);{const A=c.from(f,Q0,{sampler0:this.arrowPartsTex}),o=new Q(g.arrowFrameGeom,A);o.x=32,o.y=32,o.rotation=Math.PI,this.arrowFrame=o}for(let A=0;A<10;A++){const o=c.from(f,u,{sampler0:this.arrowPartsTex,time:0,quant:Math.min(A,6)}),n=new a(g.arrowFrameTex);n.x=A%3*64,n.y=Math.floor(A/3)*64;const r=new Q(g.arrowBodyGeom,o);r.x=A%3*64+32,r.y=Math.floor(A/3)*64+32,r.rotation=Math.PI,r.name="body"+A,g.arrowContainer.addChild(n),g.arrowContainer.addChild(r)}for(let A=0;A<10;A++){const o=c.from(f,u,{sampler0:this.liftQuantsTex,time:0,quant:Math.min(A,6)}),n=c.from(f,w0,{sampler0:this.liftPartsTex}),r=new Q(g.liftBodyGeom,o);r.x=A%3*64+32,r.y=Math.floor(A/3)*64+32,r.rotation=Math.PI,r.name="body"+A,g.liftContainer.addChild(r);const B=new Q(g.liftFrameGeom,n);B.x=A%3*64+32,B.y=Math.floor(A/3)*64+32,B.rotation=Math.PI,B.name="frame"+A,g.liftContainer.addChild(B)}{const A=c.from(f,c0,{sampler0:this.minePartsTex,time:0}),o=new Q(g.mineBodyGeom,A),n=new a(l0);n.width=64,n.height=64,n.anchor.set(.5),n.pivot.y=3,g.mineContainer.position.set(32),g.mineContainer.addChild(o),g.mineContainer.addChild(n)}this.loaded=!0}}static setArrowTexTime(A){if(!this.loaded)return;const o=A.chartManager.chartView.getVisualBeat(),n=A.chartManager.chartView.getVisualTime();for(let r=0;r<10;r++){const B=g.arrowContainer.getChildByName("body"+r);B.shader.uniforms.time=o/8;const F=g.liftContainer.getChildByName("body"+r);F.shader.uniforms.time=o/8;const U=g.liftContainer.getChildByName("frame"+r);U.shader.uniforms.time=o/8}g.mineContainer.children[0].shader.uniforms.time=n,g.mineContainer.rotation=n%1*Math.PI*2,A.renderer.render(g.arrowFrame,{renderTexture:g.arrowFrameTex}),A.renderer.render(g.arrowContainer,{renderTexture:g.arrowTex}),A.renderer.render(g.mineContainer,{renderTexture:g.mineTex}),A.renderer.render(g.liftContainer,{renderTexture:g.liftTex})}static setNoteTex(A,o){if(o!==void 0&&o.type=="Mine")A.texture=g.mineTex;else if(o!==void 0&&o.type=="Lift"){const n=[4,8,12,16,24,32,48,64,96,192].indexOf(o?.quant??4);A.texture=new e(g.liftTex.baseTexture,new h(n%3*64,Math.floor(n/3)*64,64,64))}else{const n=[4,8,12,16,24,32,48,64,96,192].indexOf(o?.quant??4);A.texture=new e(g.arrowTex.baseTexture,new h(n%3*64,Math.floor(n/3)*64,64,64))}}}const d0="/smeditor/assets/flashBright-WBniwOZj.png",P0="data:image/png;base64,",D0="/smeditor/assets/hold-BtPJfgG_.png",p0="/smeditor/assets/mine-CPFI4p0C.png",h0=e.from(d0),u0=e.from(P0),T0=e.from(D0),L0=e.from(p0),D={0:{tint:16777215,alpha:1,"scale.x":.8,"scale.y":.8},"0.5":{tint:16777215,alpha:1,"scale.x":1,"scale.y":1},1:{tint:16777215,alpha:0,"scale.x":1,"scale.y":1}},T=t=>({0:{tint:t,alpha:1,"scale.x":.4,"scale.y":.4},"0.5":{tint:t,alpha:1,"scale.x":.5,"scale.y":.5},1:{tint:t,alpha:0,"scale.x":.5,"scale.y":.5}}),p={w0:D,w1:D,w2:T(16777036),w3:T(65382),held:D};class H0 extends P{holdExplosion=new H(L(T0,4,1,160,160)[0]);bright=new a(h0);dim=new a(u0);anims=new Set;constructor(A,o){super(),A.on(this,"hit",n=>{if(o==n.columnNumber){let r=this.dim;if(n.judgement.id=="w0"&&(r=this.bright),p[n.judgement.id]===void 0)return;this.anims.forEach(B=>C.finish(B)),this.anims.clear(),this.anims.add(C.animate(r,p[n.judgement.id],.12))}}),A.on(this,"held",n=>{o==n.columnNumber&&(this.anims.forEach(r=>C.finish(r)),this.anims.clear(),this.anims.add(C.animate(this.bright,p.held,.12)))}),A.on(this,"holdon",n=>{o==n.columnNumber&&(this.holdExplosion.visible=!0)}),A.on(this,"holdoff",n=>{o==n.columnNumber&&(this.holdExplosion.visible=!1)}),A.on(this,"rollon",n=>{o==n.columnNumber&&(this.holdExplosion.visible=!0)}),A.on(this,"rolloff",n=>{o==n.columnNumber&&(this.holdExplosion.visible=!1)}),A.on(this,"hitmine",n=>{if(o==n.columnNumber){const r=new a(L0);r.alpha=0,r.anchor.set(.5),r.blendMode=x.ADD,this.addChild(r),C.animate(r,{0:{alpha:1,rotation:0},"0.5":{alpha:1,rotation:90*Math.PI/180},1:{alpha:0,rotation:180*Math.PI/180}},.4,void 0,()=>r.destroy())}}),this.holdExplosion.visible=!1,this.holdExplosion.anchor.set(.5),this.holdExplosion.scale.set(.5),this.holdExplosion.play(),this.addChild(this.holdExplosion),this.dim.scale.set(.5),this.dim.anchor.set(.5),this.dim.alpha=0,this.addChild(this.dim),this.bright.scale.set(.5),this.bright.anchor.set(.5),this.bright.alpha=0,this.addChild(this.bright)}}const F0="/smeditor/assets/receptor-SrJwFcbg.png",U0=e.from(F0),i={},s={};for(const t of["Left","Down","Up","Right","UpLeft","UpRight"])for(const A of["Body","BottomCap"])for(const o of["Active","Inactive"])i[t]===void 0&&(i[t]={}),i[t][o]===void 0&&(i[t][o]={}),i[t][o][A]=e.from(new URL(Object.assign({"./hold/downBodyActive.png":b,"./hold/downBodyInactive.png":G,"./hold/downBottomCapActive.png":S,"./hold/downBottomCapInactive.png":y,"./hold/leftBodyActive.png":Y,"./hold/leftBodyInactive.png":N,"./hold/leftBottomCapActive.png":V,"./hold/leftBottomCapInactive.png":O,"./hold/rightBodyActive.png":X,"./hold/rightBodyInactive.png":K,"./hold/rightBottomCapActive.png":k,"./hold/rightBottomCapInactive.png":R,"./hold/upBodyActive.png":M,"./hold/upBodyInactive.png":Z,"./hold/upBottomCapActive.png":J,"./hold/upBottomCapInactive.png":z,"./hold/upleftBodyActive.png":q,"./hold/upleftBodyInactive.png":j,"./hold/upleftBottomCapActive.png":W,"./hold/upleftBottomCapInactive.png":_,"./hold/uprightBodyActive.png":$,"./hold/uprightBodyInactive.png":A0,"./hold/uprightBottomCapActive.png":n0,"./hold/uprightBottomCapInactive.png":t0})[`./hold/${t.toLowerCase()}${A}${o}.png`],import.meta.url).href);for(const t of["body","bottomCap"])for(const A of["Active","Inactive"])s[A]===void 0&&(s[A]={}),s[A][t]=e.from(new URL(Object.assign({"./roll/bodyActive.png":o0,"./roll/bodyInactive.png":g0,"./roll/bottomCapActive.png":r0,"./roll/bottomCapInactive.png":e0})[`./roll/${t}${A}.png`],import.meta.url).href);const x0={Left:90,Down:0,Up:180,Right:-90,UpLeft:135,UpRight:-135,DownRight:-45,DownLeft:45},b0=["Receptor","Tap","Lift","Fake","Hold Inactive Head","Hold Active Head","Roll Inactive Head","Roll Active Head","NoteFlash"],N0={elements:{Left:{Receptor:t=>{let A;const o=L(U0,2,1,256,256),n=new H(o[0]);return n.width=64,n.height=64,n.anchor.set(.5),t.noteskin.on(n,"ghosttap",r=>{r.columnNumber==t.columnNumber&&(C.finish(A),A=C.animate(n,{0:{alpha:1.2,width:48,height:48},1:{alpha:1,width:64,height:64}},.06))}),t.noteskin.onUpdate(n,r=>{const B=(r.getVisualBeat()%1+1)%1;n.currentFrame=B<.2?0:1}),n},Tap:t=>{const A=new a(e.WHITE);return g.setNoteTex(A,t.note),A.anchor.set(.5),A},NoteFlash:t=>new H0(t.noteskin,t.columnNumber),Fake:{element:"Tap"},Lift:{element:"Tap"},Mine:{element:"Tap"},"Hold Active Head":{element:"Tap"},"Hold Inactive Head":{element:"Tap"},"Hold Active Body":t=>new d(i[t.columnName].Active.Body),"Hold Inactive Body":t=>new d(i[t.columnName].Inactive.Body),"Hold Active TopCap":()=>new a(e.EMPTY),"Hold Inactive TopCap":()=>new a(e.EMPTY),"Hold Active BottomCap":t=>new l(i[t.columnName].Active.BottomCap),"Hold Inactive BottomCap":t=>new l(i[t.columnName].Inactive.BottomCap),"Roll Active Head":{element:"Tap"},"Roll Inactive Head":{element:"Tap"},"Roll Active Body":()=>new d(s.Active.body),"Roll Inactive Body":()=>new d(s.Inactive.body),"Roll Active TopCap":()=>new a(e.EMPTY),"Roll Inactive TopCap":()=>new a(e.EMPTY),"Roll Active BottomCap":()=>new l(s.Active.bottomCap),"Roll Inactive BottomCap":()=>new l(s.Inactive.bottomCap)}},load:function(t,A){const o=t.columnName;t.columnName="Left";const n=this.loadElement(t,A);return b0.includes(t.element)&&(n.rotation=x0[o]*Math.PI/180),n},init(){g.initArrowTex()},update(t){g.setArrowTexTime(t.chartManager.app)},metrics:{HoldBodyBottomOffset:-32,RollBodyBottomOffset:-32},hideIcons:["Lift"]};export{N0 as default};
